<template>
  <div>
    <button
      type="button"
      class="btn btn-info btn-md falko-button-danger"
      id="deletebutton"
      data-toggle="modal"
...
export default {
  ...
  methods: {
    async deleteRevision() {
      ...
      try {
        const response = await api.get(`/revisions/${this.$route.params.id}`, {
          headers
        });
        const id = response.data.sprint_id;

        await api.delete(`/revisions/${this.$route.params.id}`, { headers });
